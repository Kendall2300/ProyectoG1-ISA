class Instruction:
  ...
  def __init__(self, instr_str: str):
    ...
    if self.op in self.R_TYPE_INSTRS:
    ...
    elif self.op in self.I_TYPE_INSTRS:
      # INSTR FORMAT: OP RD, RS1, IMM
      self.rd = tokens[1].strip()
      self.rs1 = tokens[2].strip(',')
      self.immediate = int(tokens[3].strip())

    elif self.op in self.S_TYPE_INSTRS:
    ...
    elif self.op in self.B_TYPE_INSTRS:
    ...
    # System instructions
    elif self.op in self.SYSTEM_TYPE_INSTRS:
      # NOP and HALT have no operands
      pass

    # Vault/Hash/Signature instructions
    elif self.op in self.VAULT_INSTRS:
